...
>
> 11. **Lightweight admin portal built (Commit `e140986`, on `admin-portal` branch).** Operator dashboard at `/admin` with HMAC-signed HttpOnly cookie auth (login form, not Basic Auth). 6 stat cards (runs today, success rate, active now, errors today, est. cost today, avg cost/run), runtime config editor (model / max_tokens / max_retries without Railway redeploy), 22-tool grid with toggle functionality, recent sessions table with date range / status / search filters, daily usage breakdown by model. Backend modules: `agent/src/admin/{auth,middleware,config,usage,stats,tools-catalog}.ts`. `agent/src/agent/loop.ts` modified to read config from Redis (env var fallback) and record usage fire-and-forget after each turn. 8 new routes under `/admin/*`. Frontend: `app/admin/login/page.tsx` + `app/admin/page.tsx` + `lib/admin-client.ts`. On a separate branch until verified end-to-end against a separate Railway preview deployment — won't land on main until we're confident. See MEMORY.md "Decision: lightweight admin portal on a separate branch".
>
> 12. **BRD document** is drafted in `BRD.md` (working tree, uncommitted at time of writing). Needs a final pass to include admin portal + Commit 2h + Custom App v2 details before commit.
>
> 13. For the **current state of the build**, read `CONTEXT.md`. For the **why** of every decision, read `MEMORY.md`. This plan is historical — it's Session 1's blueprint, not the running architecture.
