...
The frontend is a chat-first single-page UI with a **split layout**: user workspace on the left (40%), agent workspace on the right (60%), thin vertical rule between, pinned execution plan + progress feed sticky at the top of the agent column, collapses to a single chronological timeline below 1024px. 13px base font size (scaled down ~18.75% from the browser default for a denser dashboard feel). 10 component files rendering 14 distinct UI states (JourneyStepper, ExecutionPlanCard, PlanReviewTree, PlanIntegrityPanel, ApprovalPrompt, ApiKeyCard, FileUploadCard, ProgressFeed, ReflectionCard, CompletionCard + chat timeline renderers) that render whatever the agent emits via tool calls. No state machine, no wizard, no multi-page flow. See `frontend/components/agent-emitted/` for the full set.

There is also a **lightweight operator admin portal** at `/admin` — login form auth via HttpOnly HMAC cookie, **tabbed dashboard** with four sections: Observability (6 stat cards + usage by model), Runtime Config (live editor for model / max_tokens / 429 retries without a Railway redeploy), Agent Tools (22-tool grid grouped by function with toggle-to-disable), and Recent Sessions (filterable table with date range + status + search). The dashboard loads in ~200ms thanks to pre-computed set-based counters in Redis — stats are incremented at source-of-truth events (session creation, completion, error) instead of derived post-hoc from walking event logs. The /admin/* routes are fail-closed-gated behind `ADMIN_USERNAME` + `ADMIN_PASSWORD` env vars (return 503 if unset). See [`frontend/app/admin/`](frontend/app/admin/) and [`agent/src/admin/`](agent/src/admin/).
